...
TBool Utils::TimeToString(std::string &a_roDate, std::string &a_roTime, const TEntry &a_roEntry)
{
	...
	/* OS3 pads the returned date string with spaces, up to a seemingly arbitrary length, whereas OS4 does not, */
	/* so to ensure identical behaviour on both systems, we will strip any trailing spaces from the date string */

	TrimString(Date);

	/* Now copy the strings generated by dos.library into the strings passed in */
