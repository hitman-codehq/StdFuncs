 ...
 * Sends a message to the underlying DOS file system handler to request it to read whatever bytes are available, up
 * to a maximum size. This is an asynchronous operation. A signal will be handled in the RApplication class and the
 * readComplete() callback will be called with the data read.
 *
...
void RStdExecuter::read()
{
	...
	/* Send a read packet to he file system handler to read as much output from the child's stdout as possible. When
	/* the child exits, the pipe will be closed and Read() will fail */
...
TResult RStdExecuter::launchCommand(const char *a_commandName, int a_stackSize, Callback a_callback)
{
	...
	m_stdOutWrite = Open("PIPE:RADRunner/256/1", MODE_NEWFILE);
	m_stdOutRead = Open("PIPE:RADRunner/256/1", MODE_OLDFILE);

	if ((m_stdInRead != 0) && (m_stdOutWrite != 0) && (m_stdOutRead != 0))
	{
		int result = SystemTags(a_commandName, SYS_Input, m_stdInRead, SYS_Output, m_stdOutWrite,
			NP_ExitCode, (ULONG) ExitFunction, NP_ExitData, (ULONG) &exitCode, SYS_Asynch, TRUE, NP_StackSize, stackSize,
			TAG_DONE);

		if (result == 0)
		{
			m_port = CreateMsgPort(); // TODO: CAW - CreatePort()?
			m_packet = (DosPacket *) AllocDosObject(DOS_STDPKT, NULL);
			m_buffer = new char[STDOUT_BUFFER_SIZE];

			if (m_port != nullptr && m_packet != nullptr && m_buffer != nullptr)
			{
				retVal.m_result = KErrNone; // TODO: CAW - KErrNoMemory?
			...
			/* handle ourselves */
			m_stdInRead = m_stdOutWrite = 0; // TODO: CAW
		}
	}
